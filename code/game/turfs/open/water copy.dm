...
/turf/open/water/mire/Initialize()
	icon_state = "dirt"
	dir = pick(GLOB.cardinals)
	water_color = pick("#437044")
	.  = ..()

/turf/open/water/mire/Entered(atom/movable/AM, atom/oldLoc)
	. = ..()
	var/mob/living/carbon/human/FM = AM
	if(isseelie(FM) && !(FM.resting))	//Add wingcheck
		return
...
/turf/open/water/mire/deep/Entered(atom/movable/AM, atom/oldLoc)
	. = ..()
	var/mob/living/carbon/human/FM = AM
	if(isseelie(FM) && !(FM.resting))	//Add wingcheck
		return
